from aawedha.trainers.samtorch import enable_running_stats
from aawedha.trainers.samtorch import disable_running_stats
...
class Wasam(SAMTorch):
    def __init__(self, module=None, device='cuda', name='torchmodel', swa_start=50):
    ...
    def _fit_loop(self, train_loader, validation_data, has_validation,
        ...
        for epoch in range(epochs):  # loop over the dataset multiple times
            ...
            if epoch > self.swa_start:
                ...
                self.optimizer.bn_update(train_loader, self.module, self.device) # <-----Update batchnorm statistics
            ...
            if epoch > self.swa_start and epoch < epochs:
                # after model evaluation, swap weights with averaged weights (if training continues)
                self.optimizer.swap_swa_sgd()
