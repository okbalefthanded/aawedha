...
class LaresiHybrid:
    ...
    def _get_info(self, raw_desc, raw_pos, starts, ends):
        ...
        for i in range(len(starts)):
            ...
            pos = raw_pos[idx]
            evs = np.unique(desc) 
            stimuli = np.sum(np.logical_and(evs > Base_Stimulations, evs < OV_Target))
            ...
            if np.any(evs == OV_Target): # erp
                info['ERP'] = {}
                y = raw_desc[np.logical_or(raw_desc==OV_Target, raw_desc==OV_NonTarget)]
                y[y==OV_Target] = 1
                y[y==OV_NonTarget] = 0
                info['ERP']['desc'] = desc 
                info['ERP']['pos'] = pos
                info['ERP']['y'] = y
                info['ERP']['session_interval'] = (np.floor( [starts[i], ends[i]]) * self.fs).astype(int) 
            else: #ssvep
                y = desc
                info['SSVEP'] = {}                
                ...
                info['SSVEP']['pos'] = pos
                info['SSVEP']['y'] = y
                info['SSVEP']['session_interval'] = (np.floor( [starts[i], ends[i]]) * self.fs).astype(int)                  
               
            # session_interval = np.floor( [starts, ends] ) * self.fs # begin, end of session     
        return info
...
class LaresiEEG(DataSet):
    ...
    def load_raw(self, cnts=[], infos=[], epoch_duration=[0,.7], 
        ...
        '''
        '''
        epochs = self._get_epochs(cnts[0],infos[0], epoch_duration, band, order)        
        test_epochs, test_y = self._get_epochs_test(cnts[1], infos[1], epoch_duration, band, order)        
    ...
    def _get_epochs(self, cnt, info, epoch, band, order):
        ...
        signal = cnt[info['session_interval'][0]:info['session_interval'][1] , :]
...
class LaresiERP(LaresiEEG):
    ...
    def _get_epochs_test(self, cnt=None, markers={},
        ...
        for tr in range(len(erp_start)):
            # filter, epoch, append
            idx = np.logical_and(erp_mrk >= erp_start[tr], erp_mrk <= erp_end[tr])
            mrk = erp_mrk[idx] - erp_start[tr]  
            erp_signal = bandpass(cnt[erp_start[tr]:erp_end[tr],:], band, self.fs, order)
            erp_epochs.append(eeg_epoch(erp_signal, epoch, mrk) )

        samples, channels, trials =  erp_epochs[0].shape
        blocks = len(erp_epochs)
        eps = np.array(erp_epochs).transpose((1,2,3,0)).reshape((samples, channels, trials*blocks))
...
class LaresiSSVEP(LaresiEEG):
    ...
    def _get_epochs_test(self, cnt=None, markers={},
        ...
        for i  in range(len(ssvep_start)):
            idx1 = np.logical_and(raw_pos>ssvep_start[i],raw_pos<ssvep_end[i]) 
            idx2 = np.logical_and(raw_desc>base, raw_desc<=base+10) # 10 is an arbitary number
        ...
        for tr in range(len(ev_desc)):
            ...
            ssvep_signal = bandpass(cnt[ssvep_start[tr]:ssvep_end[tr],:], band, self.fs, order)
            ssvep_epochs.append(eeg_epoch(ssvep_signal, epoch, np.array([mrk])).squeeze())
