...
from aawedha.layers.ibn import IBN, IBN1d
from aawedha.layers.rbn import RBN2d, RBN1d
...
#
class TemporalBlockLN(nn.Module):
    def __init__(self, n_inputs, n_outputs, kernel_size, stride, dilation, padding, dropout=0.2, shape=2):
        super(TemporalBlockLN, self).__init__()
        self.conv1  = nn.Conv1d(n_inputs, n_outputs, kernel_size, stride=stride, padding=padding, dilation=dilation)
        self.chomp1 = Chomp1d(padding)
        self.bn1  = nn.LayerNorm([n_outputs, shape])
        self.elu1 = nn.ELU()
        self.dropout1 = nn.Dropout(dropout)

        self.conv2 = nn.Conv1d(n_outputs, n_outputs, kernel_size, stride=stride, padding=padding, dilation=dilation)
        self.chomp2 = Chomp1d(padding)
        self.bn2 = nn.LayerNorm([n_outputs, shape])
        self.elu2 = nn.ELU()
        self.dropout2 = nn.Dropout(dropout)
...
class TemporalConvNetLN(nn.Module):
    def __init__(self, num_inputs, num_channels, kernel_size=2, dropout=0.2, shape=2):
        super(TemporalConvNetLN, self).__init__()
        layers = []
        num_levels = len(num_channels)
        for i in range(num_levels):
            dilation_size = 2 ** i
            in_channels = num_inputs if i == 0 else num_channels[i-1]
            out_channels = num_channels[i]
            layers += [TemporalBlockLN(in_channels, out_channels, kernel_size, stride=1, dilation=dilation_size,
                                     padding=(kernel_size-1) * dilation_size, dropout=dropout, shape=shape)]
...
class EEGTCNetLN(nn.Module):

    def __init__(self, nb_classes, Chans=64, Samples=128, layers=3, kernel_s=10, filt=10, 
             dropout=0, activation='relu', pooling='avg', F1=4, D=2, kernLength=64, 
             dropout_eeg=0.1, name="EEGTCNetLN"):
        ...
        # EEGNET+LN
        self.conv1 = nn.Conv2d(1, F1, (1, kernLength), bias=False, padding='same')
        self.bn1   = nn.LayerNorm([F1, Chans, Samples])
        self.conv2 = Conv2dWithConstraint(F1, F1 * D, (Chans, 1), max_norm=1, bias=False, groups=F1, padding="valid")      
        self.bn2   = nn.LayerNorm([F1 * D, 1, Samples])
        self.pool1 = nn.AvgPool2d(kernel_size=(1, 8))
        self.drop1 = nn.Dropout(p=dropout_eeg)
        # https://discuss.pytorch.org/t/how-to-modify-a-conv2d-to-depthwise-separable-convolution/15843/7
        self.conv_sep_depth = nn.Conv2d(F2, F2, (1, 16), bias=False, groups=F2, padding="same")
        self.conv_sep_point = nn.Conv2d(F2, F2, (1, 1), bias=False, padding="valid")
        self.bn3 = nn.LayerNorm([F2, 1, Samples // 8])
        self.pool2 = nn.AvgPool2d(kernel_size=(1, 8))
        self.drop2 = nn.Dropout(p=dropout_eeg)
        shape = Samples // 64
        # TCN
        self.tcn   = TemporalConvNetLN(16, [filt]*layers, kernel_size=kernel_s, dropout=dropout, shape=shape)
        self.dense = LineardWithConstraint(filt, nb_classes, max_norm=regRate)
        #
        initialize_Glorot_uniform(self)
...
class TemporalBlockIBN(nn.Module):
    def __init__(self, n_inputs, n_outputs, kernel_size, stride, dilation, padding, dropout=0.2):
        super().__init__()
        self.conv1 = nn.Conv1d(n_inputs, n_outputs, kernel_size, stride=stride, padding=padding, dilation=dilation)
        self.chomp1 = Chomp1d(padding)
        self.bn1 = IBN1d(n_outputs)
        self.elu1 = nn.ELU()
        self.dropout1 = nn.Dropout(dropout)

        self.conv2 = nn.Conv1d(n_outputs, n_outputs, kernel_size, stride=stride, padding=padding, dilation=dilation)
        self.chomp2 = Chomp1d(padding)
        self.bn2 = IBN1d(n_outputs)
        self.elu2 = nn.ELU()
        self.dropout2 = nn.Dropout(dropout)
...
class TemporalConvNetIBN(nn.Module):
    def __init__(self, num_inputs, num_channels, kernel_size=2, dropout=0.2):
        super().__init__()
        layers = []
        num_levels = len(num_channels)
        for i in range(num_levels):
            ...
            in_channels = num_inputs if i == 0 else num_channels[i-1]
            out_channels = num_channels[i]
            layers += [TemporalBlockIBN(in_channels, out_channels, kernel_size, stride=1, dilation=dilation_size,
                                     padding=(kernel_size-1) * dilation_size, dropout=dropout)]
...
class EEGTCNetIBN(nn.Module):

    def __init__(self, nb_classes, Chans=64, Samples=128, layers=3, kernel_s=10, filt=10, 
             dropout=0, activation='relu', pooling='avg', F1=4, D=2, kernLength=64, 
             dropout_eeg=0.1, name="EEGTCNet_IBN"):
        ...
        self.drop2 = nn.Dropout(p=dropout_eeg)
        # TCN
        self.tcn   = TemporalConvNetIBN(16, [filt]*layers, kernel_size=kernel_s, dropout=dropout)
        self.dense = LineardWithConstraint(filt, nb_classes, max_norm=regRate)
        #
        initialize_Glorot_uniform(self)
...
class TemporalBlockRBN(nn.Module):
    def __init__(self, n_inputs, n_outputs, kernel_size, stride, dilation, padding, dropout=0.2):
        super().__init__()
        self.conv1 = nn.Conv1d(n_inputs, n_outputs, kernel_size, stride=stride, padding=padding, dilation=dilation)
        self.chomp1 = Chomp1d(padding)
        self.bn1 = RBN1d(n_outputs)
        self.elu1 = nn.ELU()
        self.dropout1 = nn.Dropout(dropout)

        self.conv2 = nn.Conv1d(n_outputs, n_outputs, kernel_size, stride=stride, padding=padding, dilation=dilation)
        self.chomp2 = Chomp1d(padding)
        self.bn2 = RBN1d(n_outputs)
        self.elu2 = nn.ELU()
        self.dropout2 = nn.Dropout(dropout)
...
class TemporalConvNetRBN(nn.Module):
    def __init__(self, num_inputs, num_channels, kernel_size=2, dropout=0.2):
        super().__init__()
        layers = []
        num_levels = len(num_channels)
        for i in range(num_levels):
            ...
            in_channels = num_inputs if i == 0 else num_channels[i-1]
            out_channels = num_channels[i]
            layers += [TemporalBlockRBN(in_channels, out_channels, kernel_size, stride=1, dilation=dilation_size,
                                     padding=(kernel_size-1) * dilation_size, dropout=dropout)]
...
class EEGTCNetRBN(nn.Module):

    def __init__(self, nb_classes, Chans=64, Samples=128, layers=3, kernel_s=10, filt=10, 
             dropout=0, activation='relu', pooling='avg', F1=4, D=2, kernLength=64, 
             dropout_eeg=0.1, name="EEGTCNet_RBN"):
        ...
        self.drop2 = nn.Dropout(p=dropout_eeg)
        # TCN
        self.tcn   = TemporalConvNetRBN(16, [filt]*layers, kernel_size=kernel_s, dropout=dropout)
        self.dense = LineardWithConstraint(filt, nb_classes, max_norm=regRate)
        #
        initialize_Glorot_uniform(self)
