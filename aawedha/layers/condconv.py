...
class Attention1d(nn.Module):
    def __init__(self,in_channels, num_experts, init_weight=True):
        ...
        self.net     = nn.Conv1d(in_channels, num_experts, kernel_size=1, bias=False)
        self.sigmoid = nn.Sigmoid()
...
class CondConv1d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, 
        ...
        self.K = num_experts
        self.init_weight = init_weight
        self.attention=Attention1d(in_channels=in_channels, num_experts=num_experts,
                                 init_weight=init_weight)       
    ...
    def forward(self,x):
        ...
        if self.bias is not None:
            ...
            output = F.conv1d(x, weight=aggregate_weight, bias=None, 
                            stride=self.stride, padding=self.padding, 
                            groups=self.groups*bs, 
                            dilation=self.dilation)
